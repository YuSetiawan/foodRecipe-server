...
const {selectAllBookmarks, selectBookmarks, insertBookmarks, updateBookmarks, deleteBookmarks, countData, findID} = require('../model/bookmarks');
...
const bookmarksController = {
  ...
  insertBookmarks: async (req, res) => {
    ...
    insertBookmarks(data)
      .then((result) => commonHelper.response(res, result.rows, 201, 'Comment Success'))
      .catch((err) => res.send(err));
  },
  ...
  updateBookmarks: async (req, res) => {
    try {
      ...
      updateBookmarks(data)
        .then((result) => commonHelper.response(res, result.rows, 200, 'Update comment Success'))
        .catch((err) => res.send(err));
    } catch (error) {
      console.log(error);
    }
  ...
  deleteBookmarks: async (req, res, next) => {
    try {
      ...
      deleteBookmarks(bookmarks_id)
        .then((result) => commonHelper.response(res, result.rows, 200, 'Delete comment Success'))
        .catch((err) => res.send(err));
    } catch (error) {
      console.log(error);
    }
