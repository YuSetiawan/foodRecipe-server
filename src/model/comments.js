...
const selectAllComments = ({limit, offset, sort, sortby}) => {
  return Pool.query(`SELECT * FROM comments ORDER BY ${sortby} ${sort} LIMIT ${limit} OFFSET ${offset}`);
};

const selectComments = (recipes_id) => {
  return Pool.query(`SELECT comments.*, users.* FROM comments LEFT JOIN users ON comments.users_id = users.id  WHERE recipes_id = '${recipes_id}'`);
};
...
const updateComments = (data) => {
  const {comment_id, recipes_id, users_id, comment_text} = data;
  return Pool.query(`UPDATE comments SET recipes_id='${recipes_id}', users_id='${users_id}', comment_text='${comment_text}' WHERE comment_id='${comment_id}'`);
};
...
const findID = (comment_id) => {
  return new Promise((resolve, reject) =>
    Pool.query(`SELECT comments FROM comments WHERE comment_id='${comment_id}'`, (error, result) => {
      if (!error) {
        resolve(result);
      } else {
        reject(error);
