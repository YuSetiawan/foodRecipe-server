...
const insertRecipes = (data) => {
  const {recipes_id, recipes_title, recipes_ingredients, recipes_photo, recipes_video, users_id} = data;
  return Pool.query(
...
const updateRecipes = (data) => {
  const {recipes_id, recipes_title, recipes_ingredients, recipes_photo, recipes_video} = data;
  return Pool.query(`UPDATE recipes SET recipes_title='${recipes_title}', recipes_ingredients='${recipes_ingredients}' ,recipes_photo='${recipes_photo}',recipes_video='${recipes_video}' WHERE recipes_id='${recipes_id}'`);
};
...
const findUUID = (recipes_id) => {
  return new Promise((resolve, reject) =>
    Pool.query(`SELECT recipes FROM recipes WHERE recipes_id='${recipes_id}'`, (error, result) => {
      if (!error) {
        resolve(result);
      } else {
        reject(error);
